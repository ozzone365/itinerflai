...
async function generatePlan(e) {
    ...
    ФОРМАТ ХОТЕЛИ:
    HOTEL: Лукс | [Име] | https://www.booking.com/searchresults.html?ss=${dest}+[Име]&aid=${affId}
    HOTEL: Бутик | [Име] | https://www.booking.com/searchresults.html?ss=${dest}+[Име]&aid=${affId}
    HOTEL: Бюджет | [Име] | https://www.booking.com/searchresults.html?ss=${dest}+[Име]&aid=${affId}
    HOTEL: Апартамент | [Име] | https://www.booking.com/searchresults.html?ss=${dest}+[Име]&aid=${affId}

    ПРОГРАМА: За всяка точка ползвай:
    ITEM: [Икона] | [Заглавие] | [Описание 2 изречения] | https://www.google.com/maps/search/?api=1&query=${dest}+[Заглавие]`;
...
function renderUI(dest, md) {
    ...
    const hotelMatches = md.match(/HOTEL: (.*)/g) || [];
    let hotelsHtml = hotelMatches.map(line => {
        const [type, name, url] = line.replace('HOTEL: ', '').split(' | ');
        return `
        <div class="bg-white p-5 rounded-[2rem] flex justify-between items-center border border-slate-100 shadow-sm transition hover:shadow-md">
            <div><p class="text-[9px] font-black text-blue-600 uppercase mb-1">${type}</p><p class="font-bold text-slate-800 text-xs">${name}</p></div>
            <a href="${url.trim()}" target="_blank" rel="noopener noreferrer" class="bg-blue-600 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase shadow-lg hover:bg-slate-900 transition">Резервирай</a>
        </div>`;
    }).join('');

    // 2. Парсване на програма
    let programHtml = "";
    const lines = md.split('\n');
    lines.forEach(line => {
        if (line.startsWith('### Ден')) {
            programHtml += `<div class="text-3xl font-black text-slate-900 border-b-8 border-blue-600/20 mt-16 mb-8 uppercase italic pb-2">${line.replace('### ', '')}</div>`;
        } else if (line.startsWith('ITEM:')) {
            const [icon, title, desc, mapUrl] = line.replace('ITEM: ', '').split(' | ');
            programHtml += `
            <div class="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-50 mb-6 flex justify-between items-center group transition hover:border-blue-200">
                <div class="flex gap-6 items-start">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 text-sm leading-relaxed max-w-xl">${desc}</p>
                    </div>
                </div>
                <a href="${mapUrl.trim()}" target="_blank" rel="noopener noreferrer" class="w-14 h-14 bg-slate-900 text-white rounded-full flex items-center justify-center flex-shrink-0 shadow-lg group-hover:bg-blue-600 transition"><i class="fas fa-map-marker-alt text-xl"></i></a>
            </div>`;
        }
    });
    ...
    res.innerHTML = `
        <div id="pdfArea" class="max-w-5xl mx-auto pb-24 bg-slate-50/30 p-4 md:p-8 rounded-[4rem]">
            <div class="bg-slate-900 p-12 rounded-[3.5rem] text-white mb-12 flex justify-between items-center shadow-2xl border-b-[12px] border-blue-600">
                <div><h2 class="text-5xl font-black italic uppercase tracking-tighter">${dest}</h2><p class="text-xs opacity-50 tracking-[0.4em] mt-2 font-light">PREMIUM EXPERIENCE</p></div>
            ...
            <div class="mb-16 px-4">
                <h4 class="text-sm font-black text-slate-400 mb-6 uppercase tracking-[0.3em] flex items-center gap-3 italic underline decoration-blue-500 decoration-4"> ПРЕПОРЪЧАНО НАСТАНЯВАНЕ</h4>
